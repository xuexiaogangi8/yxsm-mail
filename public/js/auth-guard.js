...
(function(){
  ...
  function getRedirectTarget(){
    try{ 
      ...
      // 安全验证：防止开放重定向攻击
      const allowedPaths = ['/', '/html/app.html', '/html/mailbox.html', '/html/mailboxes.html', '/html/admin.html'];
      if (!allowedPaths.includes(redirectParam)) {
        redirectParam = '/';
      }
      ...
      // 安全处理：过滤hash中的恶意内容
      const safeHash = hashToUse.replace(/[<>"'&]/g, '');
      
      if ((redirectParam === '/' || redirectParam === '/html/app.html') && safeHash) {
        return redirectParam + safeHash;
      }
      return redirectParam;
    }catch(_){ 
      // 发生错误时，优先使用已保存的 hash
      try{ 
        const ph = sessionStorage.getItem('mf:preservedHash'); 
        if (ph) {
          const safePh = ph.replace(/[<>"'&]/g, '');
          return '/' + safePh; 
        }
      }catch(_){ }
      return location.hash ? '/' + location.hash.replace(/[<>"'&]/g, '') : '/'; 
    }
  }
